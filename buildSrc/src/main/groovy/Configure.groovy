...
final class Configure {
    ...
    /**
     * Returns the specified list if the mapping toolchain is Gcc, else returns an empty list.
     *
     * @param value the values to return if the mapping toolchain is Gcc
     * @return a transformer to use in provider mapping, never null.
     */
    @CompileStatic
    private static <T> Transformer<List<T>, NativeToolChain> whenGcc(List<T> values) {
        return {
            if (it in Gcc) {
                return values
            }
     ...
     */
    @CompileStatic
    private static <T> Transformer<List<T>, NativeToolChain> whenGcc(T... values) {
        return {
            if (it in Gcc) {
                return values as List<T>
            }
    ...
    /**
     * Returns the computed list if the mapping toolchain is Gcc, else returns an empty list.
     *
     * @param value the values to return if the mapping toolchain is Gcc
     * @return a transformer to use in provider mapping, never null.
     */
    @CompileStatic
    private static <T> Transformer<List<T>, NativeToolChain> whenGcc(Transformer<List<T>, Gcc> generator) {
        return {
            if (it in Gcc) {
                return generator.transform(it)
            }
    ...
    /**
     * Adds Gcc linker options.
     *
     * @param newOptions  the new options to add
     * @return a configuration closure to execute in the application or library extension
     */
    static Closure addGccLinkerOption(String... newOptions) {
        return { component ->
            component.binaries.configureEach(sharedLibraryOrExecutable()) {
                linkTask.configure {
                    linkerArgs.addAll(toolChain.map(whenGcc(newOptions)))
                    //linkerArgs.add(0, toolChain.map(whenGcc(newOptions)))
                }
            }
        }
    }
